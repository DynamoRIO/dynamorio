...
def parse_file(filename, cache_size, matrix_size):
    ...
    for line in reversed(list(open(filename))):
        ...
        if l == "":
            ...
            i = int(f[0][0])
            if last_miss >= 0:
                output.insert(0, [i/total_lines, inst, math.e**(-last_miss)])
                last_miss += 1
...
def parse_file_return_output(filename, cache_size, matrix_size, output):
    ...
    for line in reversed(list(open(filename))):
        ...
        if l == "":
            ...
            i = int(f[0][0])
            if last_miss >= 0:
                output.insert(0, [i/total_lines, inst, math.e**(-last_miss), cache_size.strip('k'), matrix_size])
                last_miss += 1
        elif l == "00" or l == "00 00 00" or l == "00 00 00 00":
            ()
        elif "entry type" in l:
...
fout = open("./parsed_traces/full_execution_megafile.csv", "w+")
fout.write("percentage_execution,instruction_name,cache_miss_contribution,cache_size,matrix_size"+ "\n")
for o in output:
