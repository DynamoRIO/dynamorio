...
def merge_binary_csvs(
    ...
    for cache_size in cache_sizes_in_k:
        for matrix_size in matrix_sizes:
            path_to_csv = f"{path_to_csvs_folder}/{cache_size}k_l1d_matmult_{matrix_size}_parsed_total_linear.csv"
    ...
    df_total[df_total.columns[1:]].to_csv(f"{path_to_csvs_folder}/merged_linear_all.csv", index=False)
