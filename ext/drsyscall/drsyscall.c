                           ...
                           sysarg_iter_info_t *ii)
{
    ...
    if (!drsys_ops.analyze_unknown_syscalls)
        return;
    LOG(drcontext, SYSCALL_VERBOSE, "unknown system call #"SYSNUM_FMT"."SYSNUM_FMT" %s\n",
    ...
    for (i=0; i<SYSCALL_NUM_ARG_TRACK; i++) {
        ...
        if (defined) {
            ...
            LOG(drcontext, 2, "pre-unknown-syscall #"SYSNUM_FMT"."SYSNUM_FMT": param %d == "PFX"\n",
                    ...
                    for (j=0; j<cpt->sysarg_sz[i]; j++) {
                        if (is_byte_undefined(start + j)) {
                            ...
                            if (!dr_safe_write(start + j, 1,
                                ...
                                LOG(drcontext, 1, "WARNING: unable to write sentinel value @"PFX"\n",
                                    start + j);
                                break;
                            }
                        } else if (s_at != NULL) {
                            LOG(drcontext, 2, "writing sentinel value to "PFX"-"PFX" %d %d "PFX"\n",
    ...
    for (i=0; i<SYSCALL_NUM_ARG_TRACK; i++) {
        if (cpt->sysarg_ptr[i] != NULL) {
            if (safe_read(cpt->sysarg_ptr[i], cpt->sysarg_sz[i], post_val)) {
                        ...
                        if ((drsys_ops.syscall_sentinels &&
                            ...
                            if (post_val[j] == UNKNOWN_SYSVAL_SENTINEL &&
                                ...
                                if (!dr_safe_write(pc, 1, &cpt->sysarg_val[i][j],
                                                   NULL)) {
                                    LOG(drcontext, 1, "WARNING: unable to restore app sysval @"PFX"\n",
                                        pc);
                                }
                            }
                            if (w_at != NULL) {
                                LOG(drcontext, SYSCALL_VERBOSE, "unknown-syscall #"SYSNUM_FMT
                ...
                if (w_at != NULL) {
                    LOG(drcontext, SYSCALL_VERBOSE, "unknown-syscall #"SYSNUM_FMT": param %d written "
                        PFX" %d bytes\n", (ii == NULL) ? 0 : ii->arg->sysnum.number,
...
process_pre_syscall_reads_and_writes(cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    LOG(drcontext, SYSCALL_VERBOSE, "processing pre system call #"SYSNUM_FMT"."SYSNUM_FMT" %s\n",
...
process_post_syscall_reads_and_writes(cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    LOG(drcontext, SYSCALL_VERBOSE, "processing post system call #"SYSNUM_FMT"."SYSNUM_FMT,
    ...
    for (i=0; i<MAX_ARGS_IN_ENTRY; i++) { /* not <arg_count b/c of double entries */
        ...
        if (size > pt->sysarg_known_sz[sysinfo->arg[i].param]) {
            LOG(drcontext, SYSCALL_VERBOSE, "\ttruncating out size of arg %d from "PIFX" to "PIFX"\n",
                          ...
                          drsys_arg_t *arg, drsys_iter_cb_t cb, void *user_data)
{
    ...
    LOG(drcontext, 2, "iterating over args for syscall #"SYSNUM_FMT"."SYSNUM_FMT" %s\n",
