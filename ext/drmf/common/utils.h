/* **********************************************************
 * copyright (c) 2010-2026Google, Inc.  All rights reserved.
...
#define DYNAMORIO_LIBNAME            \
    IF_WINDOWS_ELSE("dynamorio.dll", \
...
#define DRMEMORY_LIBNAME                        \
    IF_WINDOWS_ELSE(CLIENT_LIBNAME_BASE ".dll", \
                    "lib" CLIENT_LIBNAME_BASE IF_MACOS_ELSE("*.dylib*", ".so*"))
...
#define ASSERT_TRUNCATE_TYPE(var, type) \
    ASSERT(sizeof(var) == sizeof(type), "mismatch " #var " and " #type)
        ...
        do {                                                                          \
            if (!(x)) {                                                               \
                NOTIFY_ERROR("ASSERT FAILURE (thread " TIDFMT "): %s:%d: %s (%s)" NL, \
                             (dr_get_current_drcontext() == NULL                      \
                                  ? 0                                                 \
                                  : dr_get_thread_id(dr_get_current_drcontext())),    \
                             __FILE__, __LINE__, #x, msg);                            \
                if (!op_ignore_asserts)                                               \
                    drmemory_abort();                                                 \
            }                                                                         \
        } while (0)
#    define ASSERT_NOT_TESTED(msg)                                                     \
        do {                                                                           \
 ...
 */
#define BUFPRINT_NO_ASSERT(buf, bufsz, sofar, len, ...)                                 \
    do {                                                                                \
...
#define FLUSH_BUFFER(fd, buf, sofar)       \
    do {                                   \
        if ((sofar) > 0)                   \
...
#define BUFFERED_WRITE(fd, buf, bufsz, sofar, len, ...)                           \
    do {                                                                          \
        int old_sofar = sofar;                                                    \
        BUFPRINT_NO_ASSERT(buf, bufsz, sofar, len, __VA_ARGS__);                  \
                                                                                  \
...
#        define ATOMIC_INC32(x) \
            __asm__ __volatile__("lock incl %0" : "=m"(x) : : "memory")
#        define ATOMIC_DEC32(x) \
            __asm__ __volatile__("lock decl %0" : "=m"(x) : : "memory")
...
{
    int cur;
    __asm__ __volatile__("lock xaddl %1, %0" : "=m"(*x), "=r"(cur) : "1"(val) : "memory");
    return (cur + val);
}
#    elif defined(ARM)
...
#define IS_WCHAR_AT(ptr) (IS_ASCII(*((byte *)(ptr))) && *(((byte *)(ptr)) + 1) == 0)
#define IS_WCHARx2_AT(ptr) (IS_WCHAR_AT(ptr) && IS_WCHAR_AT(((wchar_t *)(ptr)) + 1))
